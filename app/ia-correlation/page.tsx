...
export default function IACorrelationPage() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const selectedData = mockInvestigations
        ...
        .map((inv) => ({
          ...
          status: "completed",
          date: inv.date,
          data: { dataPoints: inv.dataPoints },
        }))
      ...
      const response = await fetch("/api/openrouter", {
        ...
        body: JSON.stringify({
          ...
          analysisType: customPrompt || "complete",
        }),
      })
      ...
      setAnalysis({
        ...
        correlations: mockAIAnalysis.correlations.filter(
          (c) =>
            selectedInvestigations.includes(c.source) && selectedInvestigations.includes(c.target)
        ),
        riskScore: data.riskScore,
        entities: mockAIAnalysis.entities,
      })
    } catch (error) {
